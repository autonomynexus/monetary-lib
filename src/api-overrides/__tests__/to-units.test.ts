import {
  castToBigintCurrency,
  castToBigjsCurrency,
  createBigintMonetary,
  createBigjsMonetary,
  createNumberMonetary,
} from "@test-utils";
import Big from "big.js";
import { USD } from "@/currencies/iso4217/amendments/168";
import { toUnits } from "../..";

describe("toUnits", () => {
  describe("number", () => {
    const monetary = createNumberMonetary;

    describe("decimal currencies", () => {
      it("returns the amount in currency units", () => {
        const d = monetary({ amount: 1050, currency: USD });

        expect(toUnits(d)).toEqual([10, 50]);
      });
      it("returns the amount in currency units, based on a custom scale", () => {
        const d = monetary({ amount: 10_545, currency: USD, scale: 3 });

        expect(toUnits(d)).toEqual([10, 545]);
      });
      it("returns the amount in currency units, with a single trailing zero", () => {
        const d = monetary({ amount: 1000, currency: USD });

        expect(toUnits(d)).toEqual([10, 0]);
      });
      it("uses a custom transformer", () => {
        const d = monetary({ amount: 1050, currency: USD });

        expect(
          toUnits(
            d,
            ({ value, currency }) => `${value.join(".")} ${currency.code}`
          )
        ).toBe("10.50 USD");
      });
    });
    describe("non-decimal currencies", () => {
      it("returns the amount in currency units", () => {
        const GRD = { code: "GRD", base: 6, exponent: 1 };
        const d = monetary({ amount: 9, currency: GRD });

        expect(toUnits(d)).toEqual([1, 3]);
      });
      it("handles currencies with multiple subdivisions", () => {
        const GBP = { code: "GBP", base: [20, 12], exponent: 1 };
        const d = monetary({ amount: 267, currency: GBP });

        expect(toUnits(d)).toEqual([1, 2, 3]);
      });
      it("handles intermediary zero values", () => {
        const GBP = { code: "GBP", base: [20, 12], exponent: 1 };
        const d = monetary({ amount: 2, currency: GBP });

        expect(toUnits(d)).toEqual([0, 0, 2]);
      });
      it("uses a custom transformer", () => {
        const GBP = { code: "GBP", base: [20, 12], exponent: 1 };
        const d = monetary({ amount: 267, currency: GBP });
        const labels = ["pounds", "shillings", "pence"];

        expect(
          toUnits(d, ({ value }) =>
            value
              .filter((amount: number) => amount > 0)
              .map(
                (amount: number, index: number) => `${amount} ${labels[index]}`
              )
              .join(", ")
          )
        ).toBe("1 pounds, 2 shillings, 3 pence");
      });
    });
  });
  describe("bigint", () => {
    const monetary = createBigintMonetary;
    const bigintUSD = castToBigintCurrency(USD);

    describe("decimal currencies", () => {
      it("returns the amount in currency units", () => {
        const d = monetary({ amount: 1050n, currency: bigintUSD });

        expect(toUnits(d)).toEqual([10n, 50n]);
      });
      it("returns the amount in currency units, based on a custom scale", () => {
        const d = monetary({ amount: 10545n, currency: bigintUSD, scale: 3n });

        expect(toUnits(d)).toEqual([10n, 545n]);
      });
      it("returns the amount in currency units, with a single trailing zero", () => {
        const d = monetary({ amount: 1000n, currency: bigintUSD });

        expect(toUnits(d)).toEqual([10n, 0n]);
      });
      it("uses a custom transformer", () => {
        const d = monetary({ amount: 1050n, currency: bigintUSD });

        expect(
          toUnits(
            d,
            ({ value, currency }) => `${value.join(".")} ${currency.code}`
          )
        ).toBe("10.50 USD");
      });
    });
    describe("non-decimal currencies", () => {
      it("returns the amount in currency units", () => {
        const GRD = { code: "GRD", base: 6n, exponent: 1n };
        const d = monetary({ amount: 9n, currency: GRD });

        expect(toUnits(d)).toEqual([1n, 3n]);
      });
      it("handles currencies with multiple subdivisions", () => {
        const GBP = { code: "GBP", base: [20n, 12n], exponent: 1n };
        const d = monetary({ amount: 267n, currency: GBP });

        expect(toUnits(d)).toEqual([1n, 2n, 3n]);
      });
      it("handles intermediary zero values", () => {
        const GBP = { code: "GBP", base: [20n, 12n], exponent: 1n };
        const d = monetary({ amount: 2n, currency: GBP });

        expect(toUnits(d)).toEqual([0n, 0n, 2n]);
      });
      it("uses a custom transformer", () => {
        const GBP = { code: "GBP", base: [20n, 12n], exponent: 1n };
        const d = monetary({ amount: 267n, currency: GBP });
        const labels = ["pounds", "shillings", "pence"];

        expect(
          toUnits(d, ({ value }) =>
            value
              .filter((amount) => amount > 0n)
              .map((amount, index) => `${amount} ${labels[index]}`)
              .join(", ")
          )
        ).toBe("1 pounds, 2 shillings, 3 pence");
      });
    });
  });
  describe("Big.js", () => {
    const monetary = createBigjsMonetary;
    const bigjsUSD = castToBigjsCurrency(USD);

    describe("decimal currencies", () => {
      it("returns the amount in currency units", () => {
        const d = monetary({ amount: new Big(1050), currency: bigjsUSD });

        expect(toUnits(d)).toEqual([new Big(10), new Big(50)]);
      });
      it("returns the amount in currency units, based on a custom scale", () => {
        const d = monetary({
          amount: new Big(10_545),
          currency: bigjsUSD,
          scale: new Big(3),
        });

        expect(toUnits(d)).toEqual([new Big(10), new Big(545)]);
      });
      it("returns the amount in currency units, with a single trailing zero", () => {
        const d = monetary({ amount: new Big(1000), currency: bigjsUSD });

        expect(toUnits(d)).toEqual([new Big(10), new Big(0)]);
      });
      it("uses a custom transformer", () => {
        const d = monetary({ amount: new Big(1050), currency: bigjsUSD });

        expect(
          toUnits(
            d,
            ({ value, currency }) => `${value.join(".")} ${currency.code}`
          )
        ).toBe("10.50 USD");
      });
    });
    describe("non-decimal currencies", () => {
      it("returns the amount in currency units", () => {
        const GRD = { code: "GRD", base: new Big(6), exponent: new Big(1) };
        const d = monetary({ amount: new Big(9), currency: GRD });

        expect(toUnits(d)).toEqual([new Big(1), new Big(3)]);
      });
      it("handles currencies with multiple subdivisions", () => {
        const GBP = {
          code: "GBP",
          base: [new Big(20), new Big(12)],
          exponent: new Big(1),
        };
        const d = monetary({ amount: new Big(267), currency: GBP });

        expect(toUnits(d)).toEqual([new Big(1), new Big(2), new Big(3)]);
      });
      it("handles intermediary zero values", () => {
        const GBP = {
          code: "GBP",
          base: [new Big(20), new Big(12)],
          exponent: new Big(1),
        };
        const d = monetary({ amount: new Big(2), currency: GBP });

        expect(toUnits(d)).toEqual([new Big(0), new Big(0), new Big(2)]);
      });
      it("uses a custom transformer", () => {
        const GBP = {
          code: "GBP",
          base: [new Big(20), new Big(12)],
          exponent: new Big(1),
        };
        const d = monetary({ amount: new Big(267), currency: GBP });
        const labels = ["pounds", "shillings", "pence"];

        expect(
          toUnits(d, ({ value }) =>
            value
              .filter((amount) => amount > new Big(0))
              .map((amount, index) => `${amount} ${labels[index]}`)
              .join(", ")
          )
        ).toBe("1 pounds, 2 shillings, 3 pence");
      });
    });
  });
});
